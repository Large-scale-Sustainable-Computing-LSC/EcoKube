...
func (p *Policy) Score(ctx context.Context, j core.Job, nodes []core.SimulatedNode) (core.Scores, error) {
	...
	p.lastBreakdown = make(map[string]map[string]float64, len(feats))
	for _, f := range feats {
		if !f.ok {
			continue
		}
		ciPenalty := math.Pow(f.ciCostG, 1.1) // convex penalty
		...
		massNorm := massZ(f.mass)
		wEff := p.W.Carbon
		if p.AlphaMass > 0 {
			wEff = wEff * (1.0 + p.AlphaMass*massNorm)
		}
		carbonTerm := wEff * ciPenalty
		waitTerm := p.W.Wait * wz
		utilTerm := p.W.Util * uz
		score := carbonTerm + waitTerm + utilTerm
		sc[f.id] = score
		p.lastBreakdown[f.id] = map[string]float64{
			"ci_cost_g":           f.ciCostG,
			"ci_penalty":          ciPenalty,
			"ci_weight_effective": wEff,
			"term_carbon":         carbonTerm,
			"wait_seconds":        f.waitS,
			"wait_norm":           wz,
			"term_wait":           waitTerm,
			"util_proxy":          f.utilProxy,
			"util_norm":           uz,
			"term_util":           utilTerm,
			"mass":                f.mass,
			"mass_norm":           massNorm,
...
// // buildScaler uses themisbase.RobustScalingCfg (not core.*)
