...
docker build "$REPO_ROOT" -f "$REPO_ROOT/hetsched/controller/Dockerfile" -t "goncaloferreirauva/ci-aware-controller:latest"
docker push "goncaloferreirauva/ci-aware-controller:latest"

# Build workload replayer
echo "Building workload-replayer..."
docker build "$REPO_ROOT" -f "$REPO_ROOT/hetsched/workloads/Dockerfile" -t "goncaloferreirauva/workload-replayer:latest"
