...
# Heuristics for column names across trace variants
CANDIDATE_COLS = {
...
def to_gi(mem_val):
    ...
    # assume bytes if a big int
    try:
        b = float(s)
        gi = max(b / (1024**3), 0.25)
        return f"{gi:g}Gi"
    except:
        # fallback: numbers like "4"
        v = parse_float(s, 1.0)
        return f"{max(v,0.25):g}Gi"
...
def main():
    # in-cluster first; fall back to local kubeconfig
